...
class Purchases:
    def init(self, debug=True):
        ...
        self.password = getpass.getpass("PASSWORD: ") or os.environ.get('PASSWORD')
        ...
        print("\r")
        self.date = input("Date [dd.mm.aaaa]: ") or datetime.strftime(datetime.strptime(str(date.today()), "%Y-%m-%d")+timedelta(days=7), '%d.%m.%Y')
    ...
    def timer(self, timer, message="Loading data: "):
        for t in range(timer):
            sys.stdout.write('\r')
            sys.stdout.flush()
            sys.stdout.write((str(message) + str(timer - t) + ' seconds').ljust(50) + '\r')
            time.sleep(1)
    ...
    def access_purchasing(self):
        ...
        current_url = self.browser.current_url
        self.browser.get(os.environ.get('MAIN_URL'))
    ...
    def article(self):
        self.timer(5)
        if (self.kind_of_basket == "material"):
            WebDriverWait(self.browser, 50).until(EC.presence_of_element_located((By.ID, "WD02FB")))
            self.browser.find_element_by_id('WD02FB').send_keys(self.article_code)
            self.browser.find_element_by_id('WD014B').click()
        else:
    ...
    def access_cards(self):
        self.browser.get(os.environ.get('PETITION_URL'))
        WebDriverWait(self.browser, 50).until(EC.presence_of_element_located((By.ID, "WDC6-contentTBody")))
